...
import FilterProcessByParamsService from "../filterProcessByParamsService";
...
describe("List process by params service", () => {
  let client: IClient;
  ...
  let sut: FilterProcessByParamsService;
  ...
  beforeEach(async () => {
    ...
    sut = new FilterProcessByParamsService(processRepository);
    ...
    await createNewProcessService.exec(client.id, {
      ...mockProcess,
  ...
  it("should list all process if no query has been provided", async () => {
    const filterProcess = await sut.exec({ page: "1", perPage: "5" });
      ...
      expect.objectContaining({
        ...
        list: expect.arrayContaining([
          expect.objectContaining({
            ...
            participantProcess: expect.arrayContaining([
              expect.objectContaining({
  ...
  it("should list all process by client id", async () => {
    const filterProcessByClientId = await sut.exec({
      page: "1",
      perPage: "5",
      client: client.id,
    });

    expect(filterProcessByClientId).toEqual(
      expect.objectContaining({
        page: 1,
        perPage: 5,
        list: expect.arrayContaining([
  ...
  it("should list all process by participant id", async () => {
    const filterProcessByClientId = await sut.exec({
      page: "1",
      perPage: "5",
